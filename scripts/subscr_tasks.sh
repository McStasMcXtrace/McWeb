...
if [ -f /srv/mcweb/McWeb/mcsimrunner_ng/new_signups.csv ]; then
    YEARDATE="$(python /srv/mcweb/McWeb/scripts/gettodaystring.py)"
    ...
    mailx -s "mcweb subscriptions: new_${YEARDATE}.csv" jaga@fysik.dtu.dk < /srv/mcweb/McWeb/scripts/subscr_email
else
